...
Here I was interested in finding the optimal number of leaves in the tree and here I used the validation set to measure the performance. Figure \ref{fig:plot23} shows how the deviance varies with increasing number of leave nodes in the decision tree. We can see as the number of leaves increases the deviance decreases for the train set which indicate that the model starts overfitting the data. The deviance on the validation set tells a different story and the optimal number of leaves, i.e. yield the lowest deviance, is 4.
