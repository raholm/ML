...
## 3
We used non-parametric bootstrap in order to estimate the 95\% confidence band for the regression tree which can be seen in figure \ref{fig:13conf}.

```{r 13conf, echo=FALSE, fig.cap="The confidence band estimated by non-parametric bootstrap for the regression tree."}
library(boot)
...
## 4
Here we used the parametric bootstrap where we have assumed that $Y \sim \mathcal{N}(\mu_{i}, \sigma^2)$ where $\mu_{i}$ are the values in the tree leaves and $\sigma^2$ is the residual variance.

```{r 14confbands, echo=FALSE, warning=FALSE, eval=TRUE, fig.cap="The 95\\% confidence and prediction bands found by parametric bootstrap after 1000 samples. The red band is the confidence band and the blue band is the prediction band."}
...
Figure \ref{fig:24msep} shows how the mean squared error (MSE) changes as the number of components are increased. It is reasonable to select either 7 or around 17 components to have in the model since the MSE barely decrease as more components are added to the model. The choice of 7 or 17 components depends on the trade-off between a simpler model versus likely better predictions.
