...
The fitted values can be seen in figure \ref{fig:12treefit} and they have a decent resemblance of the original data but the model seems to be too simplistic to capture the underlying process that generated the data set.
...
We can see from figure \ref{fig:12treeresid} that the residuals have approximately a Gaussian distribution or some form of right-tailed Gamma distribution. Since there are only 48 observations in the data set it is difficult to get a good approximation of the underlying distribution.
...
```{r 13confbands, echo=FALSE, eval=TRUE, cap.fig="The confidence bands found by non-parametric bootstrap after 1000 samples."}
<<assign1-3-confbands>>
```
