...
```{r 22trace, echo=FALSE, eval=TRUE, fig.cap="Traceplot of the first two principal components."}
<<assign2-2-trace>>
```

From the traceplot in figure \ref{fig:22trace} we can observe that the first and second PCs are combinations of basically all the variables to different extent. The first PC has relatively high combination of all variables but slighly less of the last ones while the second PC is mostly a combination of the last ones.
