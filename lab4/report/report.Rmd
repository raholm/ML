...
In this exercise I have used non-parametric bootstrap in order to approximate the 95\% confidence band for the regression model.

```{r 13confbands, echo=FALSE, eval=TRUE, fig.cap="The confidence band found by non-parametric bootstrap after 1000 samples."}
<<assign1-3-confbands>>
```

The confidence band is quite wide in figure \ref{fig:13confbands} which indicates that the model cannot capture the data particular well since it is unsure where its mean prediction should be. The model is more certain in the middle values of the MET feature and very uncertain at low values of MET. The model is overall rather unreliable and too simplistic as mentioned previously.

We can also see that the confidence bounds are bumpy which I would assume have to do with the data density in our data set. Since we only have 48 observations it is not possible to have great resolution of the confidence bounds but by increasing the number of bootstrap samples it will probably become more smooth. However, it will never be completely smooth since there are simply not enough data to be able to capture the models uncertainty for all values of MET.
...
```{r 14confbands, echo=FALSE, eval=TRUE, fig.cap="The confidence and prediction bands found by parametric bootstrap after 1000 samples. The red bands are the confidence band and the blue bands are the prediction band."}
<<assign1-4-confbands>>
```

We can see from figure \ref{fig:14confbands} that, as expected, the prediction band is wider than the confidence band because it represents the uncertainty of the data itself, not the uncertainty of the population mean. Compared to the previous result, the confidence band is narrower and much more consistent over the whole range of the MET feature. The band is also a lot smoother than previously but still pretty wide which indicates that the regression model is not very well suited for this data set. Two observations are outside the prediction band and 5\% of 48 is approximately 2 so it is as we would expect.
...
By computing $W^\prime = KW$ we get the variable loadings  which correspond to the eigenvectors in PCA.
